...
import { getRegistrationErrorMessage } from '../services/apiError'
...
export function RegisterPage() {
  const navigate = useNavigate(); const { register: createAccount } = useAuth(); const [error, setError] = useState('')
  ...
  const onSubmit = async ({ confirmPassword: _confirmPassword, ...data }: RegisterForm) => { setError(''); try { await createAccount(data); navigate('/dashboard', { replace: true, state: { success: 'Usuário criado com sucesso. Sua sessão já está ativa.' } }) } catch (submitError) { setError(getRegistrationErrorMessage(submitError)) } }
